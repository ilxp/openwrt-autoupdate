...
upgrade_fw_skip = s:option (Button, "_upgrade_fw_skip", translate("Upgrade Firmware"),translate("Upgrade with skip-SHA256-verify keeping System-Config"))
upgrade_fw_skip.inputtitle = translate ("Do Upgrade")
upgrade_fw_skip.write = function()
	luci.sys.call ("autoupdate -u --skip-verify > /dev/null &")
	luci.http.redirect(luci.dispatcher.build_url("admin", "system", "autoupdate", "log"))
end

upgrade_fw_nskip = s:option (Button, "_upgrade_fw_nskip", translate("Upgrade Firmware"),translate("Upgrade with skip-SHA256-verify without keeping System-Config"))
upgrade_fw_nskip.inputtitle = translate ("Do Upgrade")
upgrade_fw_nskip.write = function()
	luci.sys.call ("autoupdate -u -n --skip-verify > /dev/null &")
	luci.http.redirect(luci.dispatcher.build_url("admin", "system", "autoupdate", "log"))
end
